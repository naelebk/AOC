...
def solve_heat_loss(matrix)
  ...
  # [cost, i, j, dir, consecutive]
  queue = [[0, 0, 0, 0b00, 0]]
  seen_states = {}
  while queue.any?
    queue.sort_by! { |state| state[0] }
    ...
    allowed_turns(dir).each do |new_dir|
      ...
      queue << [new_cost, new_i, new_j, new_dir, new_consecutive]
    end
  end
  nil
end
...
# Execution: 23.721132146 secondes
# Pas ma version la plus opti... mais ça fonctionne c'est l'essentiel
